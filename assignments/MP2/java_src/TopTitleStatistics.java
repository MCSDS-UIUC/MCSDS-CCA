...
public class TopTitleStatistics extends Configured implements Tool {
    ...
    public static class TitleCountMap extends Mapper<Object, Text, Text, IntWritable> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			while (tokenizer.hasMoreTokens()) {
				...
				String modifiedString = nextToken.trim().toLowerCase();
				if (!stopWords.contains(modifiedString)) {
					context.write(new Text(modifiedString), new IntWritable(1));
				}
			}
        }
    }
    ...
    public static class TopTitlesStatReduce extends Reducer<NullWritable, TextArrayWritable, Text, IntWritable> {
        ...
        public void reduce(NullWritable key, Iterable<TextArrayWritable> values, Context context) throws IOException, InterruptedException {
            Integer sum = new Integer(0), mean = new Integer(0), max = new Integer(0), min = new Integer(0), var = new Integer(0);
			...
			Pair< Integer, String > firstPair = countToWordMap.first();
			min = firstPair.first;
			max = firstPair.first;
			...
			Integer varSqrSum = new Integer(0);
			
			for (Pair< Integer, String > item: countToWordMap) {
				Integer itemValue = item.first;
				int diff = itemValue - mean;
